...
class Reflect {
  constructor(recursive, remove, modified_within, only_newer, file_clone) {
    ...
    this.delete = remove;
    this.modified_within =
      modified_within !== null ? Date.now() / 1000 - modified_within : false;
  ...
  async start(src, dest, exclude) {
    ...
    if (!src || !dest)
      return { res: false, err: "Parameters 'src' and 'dest' must be defined" };

    // src and dest must be string
    if (typeof src !== 'string' || typeof dest !== 'string')
      return {
        res: false,
        err: "Parameters 'src' and 'dest' must be string paths",
      };
    ...
    if (
      !(await this.prepare(src, 'src')) ||
      !(await this.prepare(dest, 'dest'))
    )
      return {
        res: false,
        err: "Parameters 'src' and 'dest' must be a directory",
      };
    ...
    return {
      res: 'Directory "' + src + '" reflected to "' + dest + '"',
      err: false,
    };
  }
  ...
  async walk(src, dest) {
    ...
    if (this.delete && files_dest.length) {
      ...
      const files = files_dest.filter(
        (file) => !index_src[file] && !this.exclude[dest + '/' + file]
      );
    ...
    files_src.forEach((path) =>
      promises.push(this.sync(src + '/' + path, dest + '/' + path))
    );
  ...
  async sync(src, dest) {
    ...
    if (!(await this.read(dest)) || this.is_different(src, dest)) {
      await copy(
        src,
        dest,
        this.file_clone ? fs.constants.COPYFILE_FICLONE : undefined
      );

      if (this.cache[src].atime) {
        return utimes(
          dest,
          this.cache[src].atime || 0,
          this.cache[src].mtime || 0
        );
      }
    }
  ...
  async remove(path) {
    ...
    await Promise.all(
      content.map((content_path) => this.remove(path + '/' + content_path))
    );
  ...
  is_different(src, dest) {
    ...
    return (
      this.cache[src].size != this.cache[dest].size || src_mtime !== dest_mtime
    );
  }
